...
Q_matrix = np.loadtxt('matrix/Q_matrix.txt')
...
class DATA(object):
    ...
    def load_data(self, path):
        ...
        C_dataArray = np.zeros((len(c_data), self.max_seqlen)) 
        for j in range(len(c_data)):
            dat = c_data[j]
            C_dataArray[j,:len(dat)] = dat
        CD_dataArray = np.zeros((len(cd_data), self.max_seqlen)) 
        for j in range(len(cd_data)):
            dat = cd_data[j]
            CD_dataArray[j,:len(dat)] = dat
        return E_dataArray, ED_dataArray, EP_dataArray, A_dataArray, AS_dataArray, HA_dataArray,CA_dataArray,IT_dataArray,C_dataArray,CD_dataArray

class HyKTNet(nn.Module):
    def __init__(self, num_dict, embedding_dict, hg, Q_matrix, dropout = 0.2):
    ...
    def forward(self, e_data, ed_data, ep_data, a_data, as_data, ha_data, ca_data, it_data, c_data, cd_data,deliver_h=None, deliver_h_list=[], train_epoch = False):
        ...
        c_embed_data = self.c_embed(c_data)      #(batch_size, seq_len, d_as) 
        cd_embed_data = self.cd_embed(cd_data)      #(batch_size, seq_len, d_as) 
        ...
        for t in range(0, seq_len):
            ...
            q_e = self.q_matrix[eid].view(batch_size, 1, -1)   
...
def train_one_epoch(net, retain_h, optimizer, criterion, batch_size, e_data, ed_data, ep_data, a_data, as_data, ha_data, ca_data, it_data, c_data, cd_data):
    net.train()
    ...
    for idx in tqdm.tqdm(range(n), 'Training'):
        ...
        cd_one_seq  = cd_data[idx * batch_size: (idx + 1) * batch_size, :]
        ...
        input_cd = torch.from_numpy(cd_one_seq).long().to(device)
        
        target = torch.from_numpy(a_one_seq).float().to(device)

        pred_s, pred_main = net(input_e, input_ed, input_ep, input_a, input_as, input_ha, input_ca, input_it, input_c, input_cd,deliver_h=None, train_epoch = True, deliver_h_list=retain_h)
...
def test_one_epoch(net, batch_size, retain_h, shuffled_ind, e_data, ed_data, ep_data, a_data, as_data, ha_data, ca_data, it_data, c_data, cd_data):
    net.eval()
    n = int(math.ceil(len(e_data) / batch_size ))
    ...
    for idx in tqdm.tqdm(range(n), 'Testing'):
        ...
        with torch.no_grad():
            pred_s, pred_main = net(input_e, input_ed, input_ep, input_a, input_as, input_ha, input_ca, input_it, input_c, input_cd, deliver_h = retain_h_one_seq, train_epoch = False, deliver_h_list=[])
    ...
    print(all_pred.min())
    print(all_pred.max())
    print(all_target.min())
    print(all_target.max())
    loss = binary_entropy(all_target, all_pred)
    auc = compute_auc(all_target, all_pred)
...
class HyKT(nn.Module):
    def __init__(self, num_dict, embedding_dict, hg, Q_matrix, batch_size, dropout = 0.2):
        super(HyKT, self).__init__()
        Q_matrix = torch.from_numpy(Q_matrix).float().to(device)
        
        self.hykt_net = HyKTNet(num_dict, embedding_dict, hg, Q_matrix, dropout = dropout).to(device)
...
hykt = HyKT(num_dict, embedding_dict, hg, Q_matrix = Q_matrix, batch_size = batch_size, dropout = dropout)
