...
class HyKTNet(nn.Module):
    ...
    def forward(self, e_data, ed_data, ep_data, a_data, as_data, ha_data, ca_data, it_data,deliver_h=None, deliver_h_list=[], train_epoch = False):
        ...
        pred_e = torch.zeros(batch_size, seq_len).to(device)  
...
best_train_auc, best_valid_auc, train_loss_list, train_auc_list, test_loss_list, test_auc_list = hykt.train(train_data, test_data, epoch=40, lr=0.002, lr_decay_step=10)
